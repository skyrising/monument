...
interface MappingProvider {
    ...
    fun getMappings(version: String, target: MappingTarget, cache: Path = CACHE_DIR.resolve("mappings")): CompletableFuture<EntryTree<EntryMapping>?>
    fun supportsVersion(version: String, target: MappingTarget, cache: Path = CACHE_DIR.resolve("mappings")): CompletableFuture<Boolean> = getMappings(version, target, cache).thenApply { it != null }
    ...
    companion object {
        val MOJANG = object : CommonMappingProvider("mojang", MappingFormat.PROGUARD, "txt") {
            override fun getUrl(cache: Path, version: String, target: MappingTarget): CompletableFuture<URL?> =
...
abstract class CommonMappingProvider(override val name: String, override val format: MappingFormat, private val ext: String) : MappingProvider {
    abstract fun getUrl(cache: Path, version: String, target: MappingTarget): CompletableFuture<URL?>
    ...
    override fun getMappings(version: String, target: MappingTarget, cache: Path) = getUrl(getPath(cache, version), version, target).thenApply { url ->
        if (url == null) return@thenApply null
        val mappingsFile = getPath(cache, version).resolve("mappings-${target.id}.${ext}")
        download(url, mappingsFile).join()
        format.read(mappingsFile, ProgressListener.none(), null)
    }

    override fun toString(): String = "CommonMappingProvider($name)"
}
...
fun mapJar(version: String, input: Path, output: Path, mappings: EntryTree<EntryMapping>) = supplyAsync {
    ...
    jar.write(output, ProgressListener.none())
}